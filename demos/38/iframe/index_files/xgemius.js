...
if (typeof gemius_hcconn == "undefined") {
	gemius_hcconn = {
		...
		hc : (typeof gemius_hitcollector != 'undefined')?gemius_hitcollector:(typeof pp_gemius_hitcollector != 'undefined')?pp_gemius_hitcollector:'gaby.hit.gemius.pl',
		...
		load_fpdata : function() {
			if (gemius_hcconn.waiting_for_consent==0) {
				...
				var url = (gemius_hcconn.ssl?'https://':'http://')+gemius_hcconn.hc+'/fpdata.js?href='+domain;
		...
		init : function() {
			...
			if (gemius_hcconn.dnt==0) {
				if (gemius_hcconn.use_cmp && gemius_cmpclient.find_cmp()) {
					...
					gemius_hcconn.cmpto = setTimeout(gemius_hcconn.consentto,10000);
					var url = (gemius_hcconn.ssl?'https://':'http://')+gemius_hcconn.hc+'/gdprdata.js' + gemius_hcconn.gdpr_params(true);
