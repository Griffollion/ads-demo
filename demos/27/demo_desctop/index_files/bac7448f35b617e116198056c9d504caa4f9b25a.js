...
(function () {
    ...
    $('.select2').select2({
        dropdownAutoWidth: true, "language": {
            "noResults": function () {
    ...
    $('.select2_clear').select2({
        dropdownAutoWidth: true, allowClear: true, "language": {
            "noResults": function () {
                return "Результаты не найдены";
            }
        }, placeholder: function () {
    ...
    $('.select-new-style').select2({theme: 'upd-sel', width: '100%', minimumResultsForSearch: -1});
...
function city_alias() {
    var city_alias = $('#filter-cities').val(), clear_city_alias = city_alias;
...
$(document).ready(function () {
    $(document).on("pjax_filter", function () {
        ...
        var formData = $("#card-form-top :input,#card-form-sidebar :input ").filter(function (index, element) {
        ...
        $.pjax.reload({
            container: "#pj_" + id,
            url: "/" + alias + subtype_alias() + city_alias(),
            data: $.param(formData)
        });
        setTimeout(function () {
            var product_count = $('.count_product').val(), banks_count = $('.count_banks').val(),
                product_summ = $('.amount_product').val(), product_period = $('.period_product').val(),
                product_remainder = $('.remainder_product').val(),
                currency_iname = $('.filter-currency option:selected').text().toLowerCase();
            amount_text = '', other_text = '';
        ...
        var param_val = $(this).val(), param_name = $(this).data('name');
        ...
        document.cookie = param_name + "=" + param_val + ";";
        $(document).trigger('pjax_filter');
    }).off('pjax:end').on('pjax:end', function (e, resource, resp) {
        ...
        var tooltipContent = $(this).find('.g-tooltip__desc'), tooltipContentWidth = 120,
            tooltipContentOffsetLeft = tooltipContent.offset().left,
            tooltipContentAllCords = tooltipContent[0].getBoundingClientRect(), devideWidth = $(window).width(),
...
$(document).ready(function () {
    function multiselCounter(el) {
        var chosedElsContainer = el.closest('.multisel-count').siblings('.select2-container--upd-sel');
        chosedElsContainer.find('.select2-selection__rendered').hide();
        ...
        if (counter > 1) {
            chosedElsContainer.find('.select2-selection__rendered').after("<div class='multisel-count__counter'>Выбрано: " + counter + "</div>");
        } else {
            chosedElsContainer.find('.multisel-count__counter').remove();
            chosedElsContainer.find('.select2-selection__rendered').show();
        }
    }
...
;$(document).ready(function () {
    ...
    function manualInput(el) {
        ...
        if (el.siblings(".fragmentation-slider").length > 0) {
            el.siblings(".fragmentation-slider").slider('setValue', amount);
        }
    ...
    $(document).on("keydown", ".numeric", function (event) {
        ...
        if ((event.keyCode === 13)) {
            ...
            return true;
        } else if (event.keyCode === 188 || event.keyCode === 46 || event.keyCode === 8 || event.keyCode === 9 || event.keyCode === 27 || (event.keyCode === 65 && event.ctrlKey === true) || (event.keyCode >= 35 && event.keyCode <= 39)) {
            return true;
        } else {
        ...
        var value = slideEvt.type === 'change' ? slideEvt.value.newValue : slideEvt.value;
...
;yii.validation = (function ($) {
    var pub = {
        isEmpty: function (value) {
            ...
            var valid = true, regexp = /^((?:"?([^"]*)"?\s)?)(?:\s+)?(?:(<?)((.+)@([^>]+))(>?))$/,
                matches = regexp.exec(value);
            if (matches === null) {
                valid = false;
            } else {
                var localPart = matches[5], domain = matches[6];
...
$(document).on('pjax:start', function (e) {
    ...
    $(e.target).find('tbody').removeClass('loading');
    var paginationOffset = $(resp.container).offset().top, pageTopBlock = $(resp.container).data('top-block');
    ...
    if (!$(resp.container).find('.pjax-end_off-scroll').length) {
        $("html, body").stop(true).animate({scrollTop: paginationOffset}, scrollPageTop);
    }
    if (resource && typeof resource.responseText !== "undefined") {
        var title = resource.responseText.match(/<title>(.+)<\/title>/i),
            links = resource.responseText.match(/<link.*?>/gi);
    ...
    var elem = $(this), container = elem.closest('[data-pjax-container]');
    if (elem.data('url')) {
        $.pjax.reload('#' + container.attr('id'), {
            timeout: false,
            url: atob(elem.data('url')),
            push: !elem.parent().hasClass('without_history'),
            replace: false
        });
    }
